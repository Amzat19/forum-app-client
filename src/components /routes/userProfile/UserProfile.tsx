...
const UserProfile = () => {
    ...
    useEffect(() => {
        if (user) {
            ...
            const threadItemList = user.threadItems?.map((ti: ThreadItem) => {
                const threadItemsBody = JSON.parse(ti.body)[0].children[0].text;
                return (
                    <li key={`user-ti-${ti.id}`}>
                        <Link to={`/thread/${ti.thread?.id}`} className="userprofile-link">
                            {threadItemsBody <= 40 ? threadItemsBody : threadItemsBody.substring(0, 40) + " ..."};
                        </Link>
                    </li>
                )
            })
    ...
    const onClickChangePassword = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        e.preventDefault();
        const { data: changePasswordData } = await execChangePassword({ variables: { newPassword: password } });
        dispatch({
            type: "resultMsg",
